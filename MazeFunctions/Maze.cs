...
namespace MazeFunctions
{
    public static class Extensions
    {
        ...
        public static CellState OppositeWall(this CellState orig) { return (CellState) (((int) orig >> 2) | ((int) orig << 2)) & CellState.Initial; }
    ...
    public class Maze
    {
        ...
        public CellState this[int x, int y] { get => _cells[x, y]; set => _cells[x, y] = value; }
        ...
        public void VisitCell(int x, int y)
        {
            ...
            foreach (var p in GetNeighbors(new Point(x, y)).Shuffle(_rng).Where(z => !(this[z.Neighbour.X, z.Neighbour.Y].HasFlag(CellState.Visited))))
            {
                this[x, y] -= p.Wall;
        ...
        public bool[,] ToBools()
        {
            var grid = new bool[_width * 2 - 1, _height * 2 - 1];
            for (var y = 0; y < _height; y++)
            {
                for (var x = 0; x < _width; x++)
                {
                    var cell = this[x, y];
                    if (cell.HasFlag(CellState.Visited))
                    {
                        grid[x * 2, y * 2] = true;
                        if (!cell.HasFlag(CellState.Bottom) && y * 2 + 1 < _height * 2 - 1)
                        {
                            grid[x * 2, y * 2 + 1] = true;
                        }

                        if (!cell.HasFlag(CellState.Top) && y * 2 - 1 >= 0)
                        {
                            grid[x * 2, y * 2 - 1] = true;
                        }

                        if (!cell.HasFlag(CellState.Left) && x * 2 - 1 >= 0)
                        {
                            grid[x * 2 - 1, y * 2] = true;
                        }

                        if (!cell.HasFlag(CellState.Right) && x * 2 + 1 < _width * 2 - 1)
                        {
                            grid[x * 2 + 1, y * 2] = true;
                        }
                    }
